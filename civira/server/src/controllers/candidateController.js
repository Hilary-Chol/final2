...
export async function applyToJob(req, res) {
  try {
    ...
    const { jobId, qualificationScore, experienceYears, profileKeywords } = req.body;
    ...
    const [applicantRows] = await pool.query(
      'SELECT id, full_name, email, resume_file_name, resume_mime_type, resume_blob FROM applicants WHERE id = ?',
      [applicantId]
    );
    ...
    const [jobRows] = await pool.query(
      'SELECT id, organization_id, status, criteria_keywords FROM jobs WHERE id = ?',
      [jobId]
    );
    ...
    const mergedKeywords = mergeKeywords(normalizeKeywords(profileKeywords), resumeData.extractedKeywords);
